...
def attention_visualize(img, attn_mat):
    return
if __name__ == '__main__':
    from torchvision import transforms
    from PIL import Image
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    net = ViT('B_16_imagenet1k', num_classes=37)
    net.to(device)
    ...
    im = Image.open("./hw3_data/p1_data/val/26_5064.jpg")
    im = transform(im)
    im.size()

    output = net(im.unsqueeze(0).to(device))
    print(net.transformer.blocks)
    print(net.transformer.blocks[-1].attn.scores.size())  # 12 x 577 x 577
