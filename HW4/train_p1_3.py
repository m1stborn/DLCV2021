...
if __name__ == '__main__':
    ...
    if args.exp1:
        config.epochs = 50
        config.n_shot = args.k
        config.val_n_shot = args.k
    ...
    if args.exp1:
        val_dataset = MiniImageDataset(args.val_csv, args.val_data_dir)

        val_sampler = CategoriesSampler(val_dataset.label, n_batch=600,
                                        n_way=config.val_n_way, n_shot=config.val_n_shot, n_query=config.val_n_query)

        val_dataloader = DataLoader(dataset=train_dataset, batch_sampler=val_sampler,
                                    pin_memory=False, num_workers=config.n_worker, worker_init_fn=worker_init_fn)
    ...
    for epoch in range(start_epoch, start_epoch + config.epochs):
        ...
        for i, batch in enumerate(val_dataloader):
            images, labels = batch[0].to(device), batch[2]
        ...
        if pre_val_acc < val_acc.item():
            ...
            save_checkpoint(checkpoint,
                            os.path.join(args.ckpt_path, "ProtoNet-{}S-{}.pt".format(config.n_shot, uid[:8])))
